...
$hidden_sites = explode(';', $hidden_sites->hidden_sites());
...
while ($site <= $nb_sites) {
    ...
    echo "<h3>";
    if ($verrou) {
        echo "<span id='icon-lock' class='pl-icon pl-icon-lock noprint' title='Le planning est vérrouillé'></span>";
    } else {
        echo "<span id='icon-lock' class='pl-icon pl-icon-unlock noprint' title='Le planning n'est pas vérrouillé'></span>";
    }
